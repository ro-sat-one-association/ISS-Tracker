...
function verificaTimpul() {
    if(document.getElementById("time") != null && document.getElementById("time_utc_now") != null){
        ...
        document.getElementById("alerta_timp").innerHTML = "<div class=\"alert alert-info\" role=\"alert\">Am actualizat poziția acum " + (delta / 60).toFixed(2) + " minute.</div>";
    }
}
...
function verificaSatelit(){
    if (lastSat != document.getElementById("mini_sat").innerHTML){
        ...
        if (document.getElementById("err_code").innerHTML == "Wrong Callsign"){
            showOKNotification('top', 'right', 'Nu s-a găsit nicio stație cu date de locație!', 'danger');
        } else {
            showOKNotification('top', 'right', 'S-a schimbat stația urmărită', 'success');
        }
    }
}
